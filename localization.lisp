...
(defparameter *key18-2* nil)
(defparameter *key18-2-2* nil)
(defparameter *key18-3* nil)
(defparameter *key19* nil)
(defparameter *key20* nil)
(defparameter *key21* nil)
(defparameter *key22* nil)
(defparameter *key22-2* nil)
...
(defun localization ()
  (if (= *language* 1)
      (and 
		 ...
		 (setf *key18* "Наконец-то я сделала это.")
		 (setf *key18-2* "Но после выигрыша я опять")
		 (setf *key18-2-2* "вернулась в свою невзрачную форму.")
		 (setf *key18-3* "Моя кошка кончилась.")
		 ...
		 (setf *key22* "Кажется, кошки становятся моим")
		 (setf *key22-2* "личным сортом героина.")
      ...
      (and 
		 ...
		 (setf *key18* "Finally I made it!")
		 (setf *key18-2* "After victory i've returned")
		 (setf *key18-2-2* "to my old pale state.")
		 (setf *key18-3* "I ran out of my cat.")
		 ...
		 (setf *key22* "Cats became")
		 (setf *key22-2* "my own kind of drug.")
